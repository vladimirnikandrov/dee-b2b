...
function Skeleton() {
  return (
    <div style={{display:"grid",gap:24}}>
      {[0,1].map(i => (
        <div key={i} style={{background:"#000",borderRadius:10,border:"1px solid #1a1a1a",padding:"24px"}}>
          <div className="da-skeleton" style={{height:14,width:160,borderRadius:10,marginBottom:10}} />
          <div className="da-skeleton" style={{height:11,width:90,borderRadius:10,marginBottom:18}} />
          <div className="da-skeleton" style={{height:11,width:"60%",borderRadius:10,marginBottom:8}} />
          <div className="da-skeleton" style={{height:11,width:"45%",borderRadius:10}} />
        </div>
      ))}
    </div>
  );
...
export default function MyOrdersView({
  ...
  return (
    <div style={base}>
      ...
      <FadeIn delay={0.1}><main className="da-pad" style={{padding:"48px 48px"}}>
          ...
          <div style={{display:"grid",gap:24}}>
            {staleWarning && (
              <div style={{padding:"12px 16px",background:"#1a1408",border:"1px solid #4a3a10",borderRadius:10,fontSize:12,color:"#eab308",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                ...
                <button className="da-btn" onClick={reloadOrders} style={{background:"transparent",border:"1px solid #6a5518",color:"#eab308",padding:"8px 16px",borderRadius:10,fontSize:10,letterSpacing:"0.08em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT}}>Retry</button>
              </div>
            )}
            {myOrders.map(order => (
              <div key={order.id} style={{background: "#000",borderRadius:10,border: order.cancelled ? "1px solid #3f1d1d" : "1px solid #2a2a2a",padding:"24px"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:16,alignItems:"start",marginBottom:16}}>
                  <div>
                    ...
                    {(() => {
                      const chip = {display:"inline-block",fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",borderRadius:10,padding:"3px 8px"};
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    <div style={{fontSize:11,color: "#8a8a8a"}}>incl. shipping &amp; VAT</div>
                  </div>
                </div>
                {editingOrderId === order.id ? (
                  <div style={{marginBottom:12,paddingBottom:12,borderBottom: "1px solid #222"}}>
                      ...
                      {order.lines.map((l,i) => {
                        ...
                        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,fontSize:11,padding:"6px 0",borderBottom:"1px solid #222"}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                            ...
                            {stock !== null && <span style={{fontSize:11,color: "#8a8a8a",whiteSpace:"nowrap"}}>(max {l.qty + stock})</span>}
                          </div>
                        </div>
                        );
                      })}
                    ...
                    <div style={{display:"flex",gap:8,marginTop:12}}>
                      ...
                      <button onClick={()=>{setEditingOrderId(null);setEditQtys({});}} style={{background:"transparent",border: "1px solid #333",color: "#eee",padding:"8px 16px",borderRadius:10,fontSize:10,cursor:"pointer",fontFamily:FONT,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.08em"}}>Cancel</button>
                    </div>
                  </div>
                ) : (
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <button className="da-btn da-btn-outline" onClick={()=>handleViewInvoice(order.id,"myorders","deposit")} style={{background:"transparent",border: "1px solid #222",padding:"10px 20px",borderRadius:10,fontSize:10,color: "#eee",cursor:"pointer",fontFamily:FONT,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all 0.25s"}}>Shipping invoice</button>
                  ...
                  {editingOrderId === order.id ? null : (
                    <>
                      ...
                      <button className="da-btn da-btn-outline" onClick={()=>repeatOrder(order)} style={{background:"transparent",border: "1px solid #222",padding:"10px 20px",borderRadius:10,fontSize:10,color: "#eee",cursor:"pointer",fontFamily:FONT,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all 0.25s"}}>Repeat order</button>
                      {order.statuses.shipped && !order.statuses.received && !order.cancelled && <button className="da-btn da-btn-outline" onClick={()=>toggleOrderStatus(order.id,"received",{skipConfirm:true})} style={{background:"#fff",border:"none",padding:"10px 20px",borderRadius:10,fontSize:10,color:"#000",cursor:"pointer",fontFamily:FONT,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all 0.25s"}}>Confirm receipt</button>}
                      {canClientCancel(order) && <button className="da-btn da-btn-outline" onClick={()=>cancelOrder(order.id,false)} style={{background:"transparent",border:"1px solid #f87171",padding:"10px 20px",borderRadius:10,fontSize:10,color:"#f87171",cursor:"pointer",fontFamily:FONT,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all 0.25s"}}>Cancel</button>}
                    </>
                  )}
                </div>
              </div>
