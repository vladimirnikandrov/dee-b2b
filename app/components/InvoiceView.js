      ...
      return (
        <div style={{...base,background:"#0a0a0a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",padding:"40px 20px",textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:24,flexWrap:"wrap",justifyContent:"center"}}>
            <button className="da-btn da-btn-outline" onClick={reloadOrders} style={{background:"transparent",border:"1px solid #333",padding:"12px 24px",borderRadius:10,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT,color:"#eee"}}>Try again</button>
            <button className="da-btn da-btn-outline" onClick={()=>{setViewingOrderId(null);setInvoiceSource(null);setView("myorders");}} style={{background:"transparent",border:"1px solid #333",padding:"12px 24px",borderRadius:10,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT,color:"#eee"}}>My orders</button>
          </div>
        </div>
      );
    }
    ...
    <div style={{...base,background: "#0a0a0a"}}>
      <div className="da-header-pad" style={{padding:"20px 48px",background: "#000",borderBottom: "1px solid #333",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <button className="da-btn da-btn-outline" onClick={handleBack} style={{background:"transparent",border: "1px solid #222",padding:"10px 24px",borderRadius:10,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT,color: "#eee",transition:"all 0.25s"}}>{invoiceSource?"← Back":"New Order"}</button>
          {<button className="da-btn da-btn-outline" onClick={()=>setView("myorders")} style={{background:"transparent",border: "1px solid #222",padding:"10px 24px",borderRadius:10,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT,color: "#eee",transition:"all 0.25s"}}>My orders</button>}
        </div>
        <button className="da-btn" onClick={handlePrint} style={{background:"#fff",color:"#000",border:"none",padding:"12px 28px",borderRadius:10,fontSize:11,fontWeight:600,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT}}>Save PDF</button>
      </div>
      <FadeIn delay={0.1}><div className="da-invoice-pad" style={{maxWidth:760,margin:"32px auto",background: "#000",borderRadius:16,padding:"48px 48px",border:"1px solid #1c1c1c",boxShadow:"0 4px 24px rgba(0,0,0,0.5)",position:"relative",overflow:"hidden"}}>
        ...
        <div>
          ...
          {cur && cur.statuses?.balance_invoiced && (
            <div style={{display:"flex",gap:8,marginBottom:20}}>
              <button onClick={()=>setInvoiceViewType("deposit")} style={{padding:"8px 16px",borderRadius:10,fontSize:10,fontWeight:invoiceViewType==="deposit"?600:400,border:invoiceViewType==="deposit"?"2px solid #fff":"1px solid #444",background:invoiceViewType==="deposit"?"#000":"transparent",color:invoiceViewType==="deposit"?"#fff":"#999",cursor:"pointer",fontFamily:FONT,textTransform:"uppercase",letterSpacing:"0.08em",transition:"all 0.2s"}}>Shipping</button>
              <button onClick={()=>setInvoiceViewType("balance")} style={{padding:"8px 16px",borderRadius:10,fontSize:10,fontWeight:invoiceViewType==="balance"?600:400,border:invoiceViewType==="balance"?"2px solid #fff":"1px solid #444",background:invoiceViewType==="balance"?"#000":"transparent",color:invoiceViewType==="balance"?"#fff":"#999",cursor:"pointer",fontFamily:FONT,textTransform:"uppercase",letterSpacing:"0.08em",transition:"all 0.2s"}}>Full invoice</button>
            </div>
          )}
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:28,flexWrap:"wrap",gap:12}}>
            <Logo style={{ height: 18 }} />
            <div style={{textAlign:"right"}}><h1 style={{fontSize:14,fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase",margin:0}}>{invoiceViewType==="balance"?"Order invoice":"Shipping invoice"}</h1><div style={{fontSize:11,color: "#8a8a8a",marginTop:3}}>{invoiceViewType==="balance"?"Full payment due":"Shipping fee"}</div></div>
          </div>
          <div className="da-invoice-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:20,fontSize:12,lineHeight:1.7}}>
            <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:6}}>From</div><div style={{fontWeight:600}}>{SELLER.legalName}</div><div style={{color: "#888"}}>{SELLER.address}</div><div style={{color: "#888"}}>CVR: {SELLER.cvr}</div><div style={{color: "#888"}}>{SELLER.email}</div><div style={{color: "#888"}}>{SELLER.phone}</div></div>
            <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:6}}>Bill To</div><div style={{fontWeight:600}}>{inv.buyer.company}</div>{inv.buyer.contact&&<div style={{color: "#888"}}>{inv.buyer.contact}</div>}<div style={{color: "#888"}}>{inv.buyer.address}</div><div style={{color: "#888"}}>{inv.buyer.zip} {inv.buyer.city}, {inv.buyer.country}</div>{inv.buyer.vat&&<div style={{color: "#888"}}>VAT: {inv.buyer.vat}</div>}<div style={{color: "#888"}}>{inv.buyer.email}</div></div>
          </div>
          <div className="da-invoice-meta" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:20,fontSize:12,padding:"12px 16px",background: "#1a1a1a",borderRadius:10}}>
            <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:3}}>Invoice No.</div><div style={{fontWeight:600}}>{invoiceViewType==="balance"?displayId:displayId+"-SHIP"}</div></div>
            <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:3}}>Date</div><div>{fmtDate(invDate)}</div></div>
            <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:3}}>Due Date</div><div>{fmtDate(due)}</div></div>
          </div>
          ...
          <div className="da-invoice-lines" style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginBottom:16,minWidth:500}}><thead><tr style={{borderBottom:"2px solid #000"}}>{["Product","SKU","Size","Qty","Unit Price","Total"].map((h,i)=>(<th key={i} style={{padding:"7px 6px",textAlign:i>=3?"right":"left",fontSize:10,textTransform:"uppercase",letterSpacing:"0.12em",fontWeight:600,color: "#9a9a9a",whiteSpace:"nowrap"}}>{h}</th>))}</tr></thead><tbody>{lines.map((l,i)=>(<tr key={i} style={{borderBottom: "1px solid #222"}}><td data-label="Product" style={{padding:"8px 8px",fontWeight:500}}>{l.product}</td><td data-label="SKU" style={{padding:"8px 8px",color: "#8a8a8a",fontSize:11}}>{l.sku}</td><td data-label="Size" style={{padding:"8px 8px"}}>{SIZE_LABELS[l.size] || l.size}</td><td data-label="Qty" style={{padding:"8px 8px",textAlign:"right"}}>{l.qty}</td><td data-label="Unit price" style={{padding:"8px 8px",textAlign:"right",color: "#9a9a9a"}}>{formatEUR(l.unitPrice)}</td><td data-label="Total" style={{padding:"8px 8px",textAlign:"right",fontWeight:600}}>{formatEUR(l.total)}</td></tr>))}</tbody></table>
          </div>
          ...
          {inv.vatInfo&&<div style={{marginTop:14,fontSize:11,color: "#8a8a8a",fontStyle:"italic"}}>{inv.vatInfo.note}</div>}
          <div style={{marginTop:20,paddingTop:16,borderTop: "1px solid #333",fontSize:11,color: "#888",lineHeight:1.7}}>
            <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.14em",color: "#8a8a8a",marginBottom:8}}>Payment details</div>
            ...
            <div style={{marginTop:14,padding:"12px 16px",background: "#1a1a1a",borderRadius:10,color:"#888",fontSize:11,lineHeight:1.6}}>{invoiceViewType==="balance"
