...
function EconomicSync({ economic, statuses, cancelled }) {
  ...
  return (
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10,fontSize:9,letterSpacing:"0.06em",textTransform:"uppercase",color:"#8a8a8a"}}>
    ...
    <div style={base}>
      ...
      <div className="da-pad" style={{padding:"48px 48px"}}>
              ...
              <div style={{marginBottom:24}}>
                {promoCodes.map((p,i) => (
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px",background: "#1a1a1a",borderRadius:8,marginBottom:8,fontSize:11}}>
                    ...
                    <button onClick={()=>deletePromoCode(p.code)} style={{background:"#f87171",color:"#fff",border:"none",padding:"6px 12px",borderRadius:6,fontSize:10,cursor:"pointer",fontFamily:FONT,fontWeight:500}}>Delete</button>
                  </div>
                ))}
              </div>
                ...
                <div style={{display:"grid",gap:12,marginBottom:12}}>
                  ...
                  <div className="da-grid-promo" style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
                    {["2 ML","20 ML","50 ML","100 ML","KIT"].map(size => {
                      ...
                      return (
                        <div key={size}><label style={{...labelStyle,fontSize:9}}>{size}</label><input className="da-input" style={{...inputStyle,fontSize:16}} type="number" placeholder={placeholderSize} value={adminPromoForm.prices[size]} onChange={e=>setAdminPromoForm({...adminPromoForm,prices:{...adminPromoForm.prices,[size]:e.target.value}})} /></div>
                      );
                    })}
                  </div>
                </div>
              ...
              <div className="da-grid-inv" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16,maxHeight:"400px",overflowY:"auto"}}>
                {PRODUCTS.map((p,pi) => p.variants.map((v,vi) => (
                  <div key={`${pi}-${vi}`} style={{padding:"12px",background: "#1a1a1a",borderRadius:8,border: "1px solid #222"}}>
                    ...
                    <div style={{fontSize:9,color: "#8a8a8a",marginBottom:8}}>{v.size}</div>
          ...
          {adminExpanded==="buyers" && (
            <div style={{borderTop: "1px solid #222",padding:"20px"}}>
              <div style={{marginBottom:24,maxHeight:300,overflowY:"auto"}}>
                ...
                {buyers.map((b) => (
                  <div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"12px",background: "#1a1a1a",borderRadius:8,marginBottom:8,fontSize:11}}>
                    <div style={{minWidth:0}}>
                      <div style={{fontWeight:600,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                        <span style={{opacity:b.deactivated_at?0.6:1}}>{b.email}</span>
                        {b.deactivated_at && <span style={{padding:"2px 7px",border:"1px solid #4a3a10",color:"#eab308",borderRadius:5,fontSize:9,letterSpacing:"0.08em"}}>NO ACCESS</span>}
                      </div>
                      <div style={{color: "#8a8a8a",fontSize:10,marginTop:3}}>
                    ...
                    {b.deactivated_at
                      ? <button className="da-btn" onClick={()=>restoreBuyer(b)} style={{background:"transparent",border:"1px solid #333",color:"#eee",padding:"7px 14px",borderRadius:8,fontSize:10,cursor:"pointer",fontFamily:FONT,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Restore</button>
                      : <button className="da-btn" onClick={()=>removeBuyer(b)} style={{background:"transparent",border:"1px solid #f87171",color:"#f87171",padding:"7px 14px",borderRadius:8,fontSize:10,cursor:"pointer",fontFamily:FONT,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Remove</button>}
                  </div>
                ))}
              ...
              <div style={{background: "#0a0a0a",padding:"16px",borderRadius:8}}>
                ...
                <div style={{fontSize:10,color: "#8a8a8a",marginTop:10,lineHeight:1.5}}>They'll get a welcome email explaining how the platform works — no password to set, they just sign in with a code.</div>
              </div>
            </div>
          )}
        </div>
              ...
              <div style={{marginBottom:24}}>
                {admins.map((a) => (
                  <div key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px",background: "#1a1a1a",borderRadius:8,marginBottom:8,fontSize:11}}>
                    <div>
                      <div style={{fontWeight:600}}>{a.email}{a.id===session?.id && <span style={{marginLeft:8,color:"#8a8a8a",fontWeight:400,fontSize:10}}>(you)</span>}</div>
                      {a.company && <div style={{color: "#888",fontSize:10,marginTop:2}}>{a.company}</div>}
                    </div>
                    {a.id !== session?.id && <button onClick={()=>removeAdmin(a.id)} style={{background:"#f87171",color:"#fff",border:"none",padding:"6px 12px",borderRadius:6,fontSize:10,cursor:"pointer",fontFamily:FONT,fontWeight:500}}>Remove</button>}
                  </div>
                ))}
              </div>
              <div style={{background: "#0a0a0a",padding:"16px",borderRadius:8}}>
                ...
                <div style={{fontSize:10,color: "#8a8a8a",marginTop:10,lineHeight:1.5}}>They'll get a welcome email — no password to set, they just sign in with a code at the Admin entry point.</div>
              </div>
            </div>
          )}
        </div>
                  ...
                  <div key={f.id} style={{padding:"12px",background: "#1a1a1a",borderRadius:8,border:"1px solid #7c2d12",fontSize:11}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                      <div>
                        ...
                        <div style={{color:"#8a8a8a",fontSize:9,marginTop:4}}>{new Date(f.created_at).toLocaleString("en-GB")}</div>
                      </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:12}}>
          ...
          <button className="da-btn" onClick={()=>exportCSV(filtered)} style={{background:"#fff",color:"#000",border:"none",padding:"11px 20px",borderRadius:10,fontSize:10,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",cursor:"pointer",fontFamily:FONT}}>Export CSV</button>
        </div>
        ...
        {ordersError ? (
          <div style={{padding:"40px",textAlign:"center",background:"#1a1a1a",borderRadius:12,lineHeight:1.7}}>
          ...
          <div style={{padding:"40px",textAlign:"center",color: "#9a9a9a",background: "#1a1a1a",borderRadius:12,fontSize:13}}>{allOrders.length === 0 ? "No orders yet." : "No orders match these filters."}</div>
        ) : (
          <div style={{display:"grid",gap:24}}>
            {/* A cancelled order used to be identical to a live one apart from
                ...
                CANCELLED all along. */}
            {filtered.map(order => (
              <div key={order.id} style={{background: order.cancelled ? "#0c0606" : "#000",borderRadius:12,border: order.cancelled ? "1px solid #4a1d1d" : "1px solid #2a2a2a",padding:"24px"}}>
                <div className="da-admin-details" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:20,marginBottom:20}}>
                  <div>
                    <div style={{fontSize:10,color: "#8a8a8a",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Order</div>
                    <div style={{fontSize:12,fontWeight:600,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      <span style={{textDecoration:order.cancelled?"line-through":"none",opacity:order.cancelled?0.7:1}}>{order.id}</span>
                      {order.cancelled && <span style={{padding:"2px 8px",border:"1px solid #f87171",color:"#f87171",borderRadius:6,fontSize:9,fontWeight:600,letterSpacing:"0.1em"}}>CANCELLED</span>}
                    </div>
                  </div>
                  <div>
                    <div style={{fontSize:10,color: "#8a8a8a",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Company</div>
                  ...
                  <div>
                    <div style={{fontSize:10,color: "#8a8a8a",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Total</div>
                    <div style={{fontSize:12,fontWeight:600}}>{formatEUR(order.totalWithVat)}</div>
                  </div>
                  <div>
                    <div style={{fontSize:10,color: "#8a8a8a",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Date</div>
                    ...
                    <div style={{display:"grid",gap:8,marginBottom:12}}>
                      {order.lines.map((l,i) => {
                        ...
                        <div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,alignItems:"center",fontSize:11}}>
                          ...
                          {stock !== null && <span style={{fontSize:9,color: "#8a8a8a"}}>(max {l.qty + stock})</span>}
                        </div>
                        );
                      })}
                    </div>
                ...
                <div className="da-order-actions" style={{display:"flex",gap:8}}>
                  ...
                  {editingOrderId === order.id ? null : (
                    <>
                      ...
                      {!order.cancelled && <button className="da-btn da-btn-outline" onClick={()=>cancelOrder(order.id,true)} style={{background:"transparent",border:"1px solid #f87171",padding:"9px 18px",borderRadius:10,fontSize:10,color:"#f87171",cursor:"pointer",fontFamily:FONT,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all 0.25s"}}>Cancel</button>}
