...
4. **e-conomic drafts can't be traced back to an order.** Balance-draft lines carry no description and the draft payload has no reference field, so a draft sitting in Dorte's agreement can't be matched to an order id by anything on it. We now record the direction that matters to us (`orders.economic_*_draft_number`), but the reverse lookup is still manual. Fixing it properly means either adding `references: { other: order.orderId }` to the draft payload — field name unverified against the live API — or putting the order id in each line's `description`, which changes what prints on invoices Dorte sends to customers. Not a unilateral call.
5. **`getOrCreateCustomer` matches customers by exact company name.** "Dee Store" and "DEE STORE" create two customer cards, and an existing card's `vatZone`/country is never updated when it's reused. The transient-failure duplicate was fixed 2026-07-26 (a failed lookup now refuses instead of creating); the name-matching weakness remains. Would need the resolved `customerNumber` persisted per buyer.

The longstanding tech-debt items previously listed here (monolith split, no TS/tests, silent sync failures) have all been worked through as of 2026-07-24 — see the dated entries below. DEE 04/05 pricing confirmed fine as-is (Vladimir, 2026-07-24) — no longer a backlog item. Backlog items 4 and 5 from the 2026-07-26 audit (free-text country, e-conomic idempotency) were resolved the same day — see below.
