...
2. **Re-run Google's DKIM verification for `maison-dee.com`.** The record is correct and published; Google is still serving a cached failure from the minutes when it was briefly wrong. One click on **Start authentication** in Admin console → Apps → Gmail → Authenticate email. Resend's own DKIM is separate and already verified.
