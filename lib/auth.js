...
export async function getSession() {
  ...
  try {
    const [user] = await sql`select id, email, role, deactivated_at from users where id = ${payload.id}`;
    if (!user) return null; // account deleted — the token is meaningless now
    // Revoking access has to end the sessions that already exist, or a
    // 30-day cookie keeps the trade price list open for a month.
...
// Ceiling on codes issued per address per hour. The 30-second gap alone still
// allows 120 mails an hour to any address that exists — enough to bury an inbox.
