...
async function getLogoBase64() {
  ...
  try {
    // Derived from LOGO_WHITE so a rename can't leave the PDF pointing at a
    // file that no longer exists (which would silently fall back to text).
    const filePath = path.join(process.cwd(), "public", LOGO_WHITE.replace(/^\//, ""));
...
export function drawInvoice(doc, order, type, logoB64) {
  ...
  if (!logoB64) {
    // Only reached if the logo file can't be read. It used to
