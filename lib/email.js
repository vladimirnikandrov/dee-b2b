...
// Hosts that mean "this really is production".
//
// `order.deeapril.com` is still in this set even though the domain is being
// retired and nothing in the product points at it any more. It was taken out
// on 2026-08-02 and put back the same evening, deliberately:
//
// Auth is passwordless. The OTP email IS the way in. If Railway's SITE_URL is
// still the old host, dropping it here makes `IS_PRODUCTION_PORTAL` false, and
// then every send is refused for every recipient — no code arrives, the login
// screen still says one was sent, and there is no way for anyone to tell. That
// is every buyer and Dorte locked out by a config value nobody has read
// recently, over a domain that works fine today.
//
// TO FINISH RETIRING IT: read SITE_URL in the Railway service, confirm it is
// https://order.maison-dee.com, then delete the second entry. The warning
// below tells you if it isn't.
const PRODUCTION_PORTAL_HOSTS = new Set(["order.maison-dee.com", "order.deeapril.com"]);
const RETIRING_HOSTS = new Set(["order.deeapril.com"]);
...
if (RETIRING_HOSTS.has(portalHost())) {
  console.warn(
    `[email] SITE_URL points at ${portalHost()}, which is being retired (registration expires 2026-12-16). ` +
    `Every link in every email will die with it — set SITE_URL to https://order.maison-dee.com.`
  );
}
...
<!-- Logo, set as text. See the note in the source. -->
<tr><td align="center" style="padding-bottom:40px;">
<div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:26px;line-height:1;font-weight:700;letter-spacing:0.02em;color:#ffffff;">DEE</div>
</td></tr>
...
<p style="margin:0 0 8px;font-size:11px;color:#999;letter-spacing:0.06em;">DA DESIGN APS · CVR 45305481 · Piniehøj 17, 2960 Rungsted Kyst, Denmark</p>
<p style="margin:0;font-size:11px;color:#999;letter-spacing:0.06em;">order@maison-dee.com · +45 25 68 88 99</p>
</td></tr>
...
<tr>
  <td style="padding:8px 0;font-size:9px;color:#999;letter-spacing:0.1em;text-transform:uppercase;border-bottom:1px solid #2a2a2a;">Item</td>
  <td style="padding:8px 0;font-size:9px;color:#999;letter-spacing:0.1em;text-transform:uppercase;text-align:center;border-bottom:1px solid #2a2a2a;">Qty</td>
  <td style="padding:8px 0;font-size:9px;color:#999;letter-spacing:0.1em;text-transform:uppercase;text-align:right;border-bottom:1px solid #2a2a2a;">Amount</td>
</tr>
${lines}
<tr>
  <td colspan="2" style="padding:12px 0 4px;font-size:11px;color:#999;">Subtotal (WSP)</td>
  <td style="padding:12px 0 4px;font-size:12px;text-align:right;font-weight:600;color:#fff;">${formatEUR(order.totalWSP || 0)}</td>
</tr>
${order.shipping ? `<tr>
  <td colspan="2" style="padding:4px 0;font-size:11px;color:#999;">Shipping (excl. VAT)</td>
  <td style="padding:4px 0;font-size:12px;text-align:right;color:#fff;">${formatEUR(order.shipping)}</td>
</tr>` : ""}
${order.vatAmount ? `<tr>
  <td colspan="2" style="padding:4px 0;font-size:11px;color:#999;">${escapeHtml(order.vatLabel || "VAT")}</td>
...
const TEMPLATES = {
  ...
  order_placed_buyer: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Company</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerCompany)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Date</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</td></tr>
      </table>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;margin-top:16px;">
        <tr><td bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;padding:12px 16px;">
          <p style="margin:0 0 4px;font-size:10px;color:#999;letter-spacing:0.1em;text-transform:uppercase;">Next Step</p>
  ...
  order_placed_admin: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Company</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerCompany)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Contact</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerContact) || "—"}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Email</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerEmail)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Country</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerCountry) || "—"}</td></tr>
        ${data.buyerVat ? `<tr><td style="font-size:11px;color:#999;padding:4px 0;">VAT</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerVat)}</td></tr>` : ""}
      </table>
      ${orderSummaryBlock(data)}
  ...
  deposit_invoiced: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Shipping Fee</td><td style="font-size:14px;font-weight:700;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.depositAmount || 0)}</td></tr>
      </table>
      ${divider()}
      ...
      ${ctaButton("View Invoice", `${PORTAL_URL}?order=${data.orderId}&invoice=deposit`)}
  ...
  deposit_paid: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Shipping Fee</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.depositAmount || 0)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Full Order Amount</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.balanceAmount || 0)}</td></tr>
      </table>
      ${divider()}
  ...
  balance_invoiced: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Amount Due</td><td style="font-size:14px;font-weight:700;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.balanceAmount || 0)}</td></tr>
      </table>
      ${divider()}
      ...
      ${ctaButton("View Invoice", `${PORTAL_URL}?order=${data.orderId}&invoice=balance`)}
  ...
  balance_paid: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Total Paid</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.totalWithVat || 0)}</td></tr>
      </table>
      ${divider()}
  ...
  shipped: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Shipping To</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerCity) || ""}, ${escapeHtml(data.buyerCountry) || ""}</td></tr>
      </table>
      ${divider()}
  ...
  order_cancelled_admin: (data) => ({
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Order</td><td style="font-size:12px;font-weight:600;text-align:right;padding:4px 0;color:#fff;">${data.orderId}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Company</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerCompany)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Contact</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${escapeHtml(data.buyerEmail)}</td></tr>
        <tr><td style="font-size:11px;color:#999;padding:4px 0;">Total Was</td><td style="font-size:12px;text-align:right;padding:4px 0;color:#fff;">${formatEUR(data.totalWithVat || 0)}</td></tr>
      </table>
      ${ctaButton("Open Admin Panel")}
  ...
  otp_login: (data) => ({
    ...
    html: baseLayout(`
      ...
      <p style="font-size:11px;color:#999;margin:0;text-align:center;">Didn't request this? You can safely ignore this email.</p>
    ...
    html: baseLayout(`
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;margin-bottom:12px;">
        <tr><td bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;padding:14px 16px;">
          <p style="margin:0 0 4px;font-size:10px;color:#999;letter-spacing:0.1em;text-transform:uppercase;">How to log in</p>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;">
        <tr><td bgcolor="#1a1a1a" style="background:#1a1a1a;border-radius:10px;padding:14px 16px;">
          <p style="margin:0 0 4px;font-size:10px;color:#999;letter-spacing:0.1em;text-transform:uppercase;">How it works</p>
...
// Render a template without sending it. Exists so the templates can be looked
// at and asserted on — the logo being a broken image in Apple Mail went
// unnoticed for a week because nothing here was ever rendered outside a real
...
export async function sendTransactionalEmail(type, data) {
  ...
  if (!sendable.ok) {
    ...
    // In local development this is the guard working as designed, and filling
    // the admin panel with those would train everyone to ignore it.
    //
    // In production it is the opposite: a blocked send there means SITE_URL is
    // wrong, and since sign-in is a code emailed to you, "wrong SITE_URL"
    // reads to every user as "the portal is broken and says nothing". This
    // used to be swallowed with a comment calling it impossible — which is
    // exactly the assumption that makes an outage invisible. It goes in the
    // panel Dorte already watches.
    if (process.env.NODE_ENV === "production") {
      recordSyncFailure({
        type: "email",
        orderId: data?.orderId || null,
        context: `${type} (blocked)`,
        error: sendable.reason,
      }).catch((e) => console.error("failed to record blocked send:", e));
    }
