...
// Anything that must never appear in a message we actually deliver. The
// `http://` check is deliberate: every real portal URL is https, so a plain
// http:// link is itself evidence the base URL came from somewhere wrong.
const FORBIDDEN_IN_BODY = /localhost|127\.0\.0\.1|\.railway\.app|\.vercel\.app|http:\/\//i;
...
function checkSendable(to, html) {
  ...
  // nothing carrying a localhost/preview URL may go out — that would mean the
  // deployed SITE_URL is wrong, or a stray absolute URL crept into a template.
  const match = html.match(FORBIDDEN_IN_BODY);
  if (match) {
    return { ok: false, reason: `rendered body contains a non-production URL (${match[0]}) — not sending to ${to}` };
  }
