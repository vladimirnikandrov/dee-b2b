...
import { isDenmark } from "@/lib/countries";
//
...
/** @typedef {{ size: string, sku: string, ean: string | null, wsp: number, rrp: number | null, label?: string }} ProductVariant */
/** @typedef {{ name: string, variants: ProductVariant[] }} Product */
...
// Shipping is quoted GROSS — the figure here is the final amount the buyer
// pays, VAT already inside it. Dorte's decision, 2026-07-26: "the stated
// shipping price always remains the final price and no VAT is added on top for
// the customer." lib/pricing.js splits it into net + VAT for the invoice and
// the accounting; the buyer's total never moves.
//
// Two rates because domestic parcels cost a fraction of what a cross-border one
// does. Everything that is not Denmark pays the standard rate — Dorte named
// "all other EU countries", and the rest of the world was already on 35 with
// nothing said about changing it.
export const SHIPPING_RATES_GROSS = { DK: 9.25, DEFAULT: 35 };

/**
 * Gross shipping charge for a destination.
 * @param {string | null | undefined} country canonical name or ISO code
 * @returns {number}
 */
export function shippingRateFor(country) {
  return isDenmark(country) ? SHIPPING_RATES_GROSS.DK : SHIPPING_RATES_GROSS.DEFAULT;
}

// Flat SKU -> variant (+ product name) lookup, used server-side to price orders.
/** @type {Record<string, ProductVariant & { product: string }>} */
