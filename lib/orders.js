...
export function toFlatOrderData(row) {
  return {
    orderId: row.id,
    date: row.created_at,
    // Who placed it — lib/economic.js uses this to look up (and remember) the
    // buyer's e-conomic customer card instead of re-matching on company name.
    userId: row.user_id,
    ...
    // Net. The gross the buyer pays is depositAmount below; the difference is
    // shippingVat. See db/migrations/008-shipping-vat.js.
    shipping: Number(row.shipping_amount),
    // The VAT sitting inside the quoted (gross) shipping charge. The PDF needs
    // it to state each document's own VAT rather than the order's total.
    shippingVat: Number(row.shipping_vat_amount || 0),
    ...
    ...(opts.includeInternal
      ? {
          economic: {
            ...
            // Drafts an edit replaced. They still exist in Dorte's real
            // accounting and have to be deleted there by hand, so the list only
            // grows — a document in someone's books does not go away because
            // our column got reassigned.
            supersededDrafts: row.economic_superseded_drafts || [],
          },
        }
      : {}),
