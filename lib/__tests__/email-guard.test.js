...
// A fresh clone runs `cp .env.local.example .env.local` (README). If that file
// ever ships a production SITE_URL, the guard silently switches itself off on
// every developer machine — which is exactly how this was broken once already,
// by the very commit that added the guard. Pin it.
describe(".env.local.example cannot disable the guard", () => {
  it("does not ship a production SITE_URL", async () => {
    const fs = await import("node:fs");
    const example = fs.readFileSync(new URL("../../.env.local.example", import.meta.url), "utf8");
    const line = example.split("\n").find((l) => /^SITE_URL=/.test(l));
    expect(line, "SITE_URL must be present in the example").toBeTruthy();
    const value = line.replace(/^SITE_URL=/, "").trim();
    const host = new URL(value).host.toLowerCase();
    expect(["order.maison-dee.com"]).not.toContain(host);
  });
});

// A retailer legitimately called "Anna (http://annashop.dk)" is not a
// deployment problem. Scanning the whole rendered body meant one buyer's
// punctuation silently suppressed their own order confirmation, the admin's
// copy and the invoice PDF — with nothing in sync_failures to show for it.
describe("buyer free text never blocks a production send", () => {
  const withText = (t) => ({ ...ORDER, buyerCompany: t, buyerContact: t });

  it("sends normally when the company name contains http://", async () => {
    const { mod, restore } = await loadWith({ SITE_URL: "https://order.maison-dee.com" });
    const res = await mod.sendTransactionalEmail("order_placed_buyer", withText("Anna (http://annashop.dk)"));
    restore();
    expect(res.blocked).toBeUndefined();
    expect(res.success).toBe(true);
  });

  it("sends normally when the company name contains localhost", async () => {
    const { mod, restore } = await loadWith({ SITE_URL: "https://order.maison-dee.com" });
    const res = await mod.sendTransactionalEmail("order_placed_admin", withText("localhost coffee & scent"));
    restore();
    expect(res.blocked).toBeUndefined();
  });

  // ...but a genuinely wrong base URL must still be caught, which is the whole
  // point of keeping the check at all.
  it("still blocks when OUR OWN links point somewhere non-production", async () => {
    const { mod, restore } = await loadWith({ SITE_URL: "https://web-production-bac70.up.railway.app" });
    const res = await mod.sendTransactionalEmail("order_placed_buyer", ORDER);
    restore();
    expect(res.blocked).toBe(true);
...
// The old domain is being retired but STILL COUNTS AS PRODUCTION, on purpose.
// Sign-in is a code emailed to you, so if Railway's SITE_URL is still the old
// host and the guard stops recognising it, every send is refused and every user
// is locked out with the login screen still claiming a code was sent. Keeping
// it here is what makes that impossible; the console warning in lib/email.js is
// what makes the misconfiguration visible instead. Remove the second host from
// PRODUCTION_PORTAL_HOSTS only after confirming SITE_URL in Railway.
describe("retired domain", () => {
  it("still sends from order.deeapril.com while the domain resolves", async () => {
    const { mod, restore } = await loadWith({ SITE_URL: "https://order.deeapril.com" });
    const res = await mod.sendTransactionalEmail("otp_login", { email: "real-customer@example.com", code: "123456" });
    expect(res.blocked).toBeUndefined();
    restore();
  });
});
